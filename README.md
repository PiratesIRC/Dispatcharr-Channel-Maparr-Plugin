...
* **Provider-Junk Normalization** (v1.26.1650854+): Strips stylized-Unicode decoration (superscript / small-capital tier markers), maps emoji used as letters (`beIN SP⚽RTS` → `SPORTS`), and removes numeric resolution markers (`720p`/`3840P`) before matching — so noisy IPTV stream names normalize to the same form as clean channel-database names. ASCII names are guaranteed untouched.
